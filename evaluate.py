...
model_final.compile(loss = "categorical_crossentropy", optimizer = optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
, metrics=["accuracy"])
...
train_datagen.flow(data.train.X, data.train.y, batch_size=32),steps_per_epoch=len(data.train.X) / 32, epochs=epochs)
...
for x_batch,y_batch in train_datagen.flow(data.train.X,data.train.y,batch_size=batch_size) :
    print("here")
    model_final.fit(x_batch, y_batch,callbacks=callbacks_list, verbose=0)
